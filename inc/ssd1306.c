...
void ssd1306_pixel_without_restriction(ssd1306_t *ssd, uint8_t x, uint8_t y, bool value){
...
void ssd1306_fill(ssd1306_t *ssd, bool value) {
    ...
    for (uint8_t y = 3; y < ssd->height-3; ++y) {
        for (uint8_t x = 3; x < ssd->width-3; ++x) {
          ssd1306_pixel_without_restriction(ssd, x, y, value);
        }
    }
}

void ssd1306_clear_rect(ssd1306_t *ssd, bool double_line) {
  // Itera por todas as posições do display

  for (uint8_t y = double_line == 1?4:3; y < ssd->height-4; ++y) {
      for (uint8_t x = double_line == 1?4:3; x < ssd->width-4; ++x) {
          ssd1306_pixel(ssd, x, y, 0);
      }
  }
}
...
void ssd1306_draw_rect(ssd1306_t *ssd,uint8_t xAxle,uint8_t yAxle){
  for(int x = xAxle;x < ssd->width - xAxle - 1;x++){
    ssd1306_pixel_without_restriction(ssd,x,yAxle,1);
    ssd1306_pixel_without_restriction(ssd,x,ssd->height - yAxle - 1,1);

    if(x < ssd->height - yAxle - 1){
      ssd1306_pixel_without_restriction(ssd,yAxle,x,1);
      ssd1306_pixel_without_restriction(ssd,ssd->width-xAxle-1,x,1);
    }
...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  if (c >= 'A' && c <= 'Z')
  {
    index = (c - 'A' + 11); // Para letras maiúsculas
  }else  if (c >= '0' && c <= '9')
  {
    index = (c - '0' + 1); // Adiciona o deslocamento necessário
  }
  else if(c >= 'a' && c <= 'z'){
    index = (c - 'a' + 37);
  }
...
void draw_square(ssd1306_t *ssd,uint8_t x, uint8_t y){
  uint8_t a[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
  for (uint8_t i = 0; i < 8; ++i)
  {
    uint8_t line = a[i];
...
void clear_rect(ssd1306_t *ssd,uint8_t xAxle,uint8_t yAxle){
  for(int x = xAxle;x < ssd->width - xAxle - 1;x++){
    ssd1306_pixel_without_restriction(ssd,x,yAxle,0);
    ssd1306_pixel_without_restriction(ssd,x,ssd->height - yAxle - 1,0);

    if(x < ssd->height - yAxle - 1){
      ssd1306_pixel_without_restriction(ssd,yAxle,x,0);
      ssd1306_pixel_without_restriction(ssd,ssd->width-xAxle-1,x,0);
    }
